-- FESTO_control v2, 10-10-2020, Florens Helfferich & Bram Pronk, TU Delft MISIT
...
-- at a speed given by RAM1. The entire script is unlocked by giving an enable on RAM4
...
--  USER_RAM2 = [46004] speed !has to be between 0 and 1.5!
--  USER_RAM4 = [46008] enable signal starts the program at a positive value
...
--  print("Main: tposV = ", tposV)
  ...
  if LJ.CheckInterval(0) then
    cposV = MB.R(0,3)
    if cposV > tposV then 
      LJ.IntervalConfig(2,30)
      LJ.IntervalConfig(3,Tvar)
      while true do
        MB.W(1002,3, offsetV-speed*1.7)
        if LJ.CheckInterval(3) then
          MB.W(1002,3, offsetV-0.1)
    ...
    elseif cposV < tposV then 
      LJ.IntervalConfig(4,30)
      LJ.IntervalConfig(5,Tvar)
      while true do
        MB.W(1002,3, offsetV+speed)
        if LJ.CheckInterval(5) then
          MB.W(1002,3, offsetV+0.1)
          if LJ.CheckInterval(4) then
            MB.W(1002,3, offsetV)
            break
          end
        end
